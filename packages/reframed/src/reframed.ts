...
export function reframed(
	...
	initializeParentExecutionContext(options.routable);
	...
	iframe.addEventListener('load', () => {
		monkeyPatchIFrameEnvironment(iframe, reframedContainerShadowRoot, options.routable);
 ...
 */
function monkeyPatchIFrameEnvironment(iframe: HTMLIFrameElement, shadowRoot: ReframedShadowRoot, routable: boolean) {
	console.log('monkeyPatchIFrameEnvironment', routable);
	assert(
	...
	if (routable) {
		...
		const nonRoutableHistoryStack: Array<{ state: any; title: string; url: string | null }> = [
			{ state: iframeWindow.history.state, title: iframeDocument.title, url: iframeWindow.location.href },
		];
		let nonRoutableHistoryCursor = 0;

		// for non-routable fragments we should always use replaceState instead of pushState so that we don't create unexpected history entries
		...
		Object.defineProperties(iframeWindow.history, {
			pushState: {
				value: function reframedNonRoutablePushState(state: any, title: string, url?: string | null) {
					if (nonRoutableHistoryCursor !== nonRoutableHistoryStack.length - 1) {
						nonRoutableHistoryStack.splice(nonRoutableHistoryCursor + 1);
					}
					nonRoutableHistoryStack.push({ state, title, url: url ?? null });
					nonRoutableHistoryCursor++;
					iframeWindow.history.replaceState(state, title, url);
				},
			},

			back: {
				value: function reframedNonroutableBack() {
					if (nonRoutableHistoryCursor === 0) return;

					nonRoutableHistoryCursor--;

					let { state, title, url } = nonRoutableHistoryStack[nonRoutableHistoryCursor];
					iframeWindow.history.replaceState(state, title, url);
				},
			},

			forward: {
				value: function reframedNonroutableForward() {
					if (nonRoutableHistoryCursor === nonRoutableHistoryStack.length - 1) return;
			...
			length: {
				get() {
					return nonRoutableHistoryStack.length;
				},
			},
		});
	}
...
function initializeParentExecutionContext(patchHistory: boolean) {
	...
	if (patchHistory && !(reframedInitializedHistoryPatchSymbol in window)) {
		window[reframedInitializedHistoryPatchSymbol] = true;
